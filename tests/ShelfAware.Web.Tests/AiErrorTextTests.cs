...
public class AiErrorTextTests
{
    ...
    public async Task Managed_with_credit_is_allowed()
    {
        var reason = await AiErrorText.BlockedReasonAsync(
            new FakeEntitlements { BalanceCredits = 5_000_000 }, Managed(), NotBlocked());
    ...
    {
        var reason = await AiErrorText.BlockedReasonAsync(
            new FakeEntitlements(HouseholdTier.Founder) { BalanceCredits = 0 }, Managed(), NotBlocked());
    ...
    public async Task Managed_Free_with_no_credit_says_subscribe()
    {
        ...
        var reason = await AiErrorText.BlockedReasonAsync(
            new FakeEntitlements(HouseholdTier.Free) { BalanceCredits = 0 }, Managed(), NotBlocked());
    ...
    public async Task Byok_with_a_key_is_allowed_and_never_consults_the_balance()
    {
        ...
        var reason = await AiErrorText.BlockedReasonAsync(
            new FakeEntitlements { BalanceCredits = 0 }, Byok("sk-visitor"), NotBlocked());
    ...
    {
        var reason = await AiErrorText.BlockedReasonAsync(
            new FakeEntitlements { BalanceCredits = 5_000_000 }, Byok(""), NotBlocked());
