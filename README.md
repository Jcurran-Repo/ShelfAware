...
confirmed before. And double-uploads get caught coming and going: a receipt that exactly matches
one already confirmed — same date, items, and prices — is never recorded automatically, it stops
and asks; and removing a receipt takes everything it recorded with it — its purchases, any
products it introduced that never gathered other history, the merchant matches it taught — so
even a confirmed mistake is one click to undo, not a permanently skewed cadence.
