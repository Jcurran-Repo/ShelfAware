...
- [ ] Unit labelling, owed from the backlog check — its Qty column renders a BARE number today (the footnote carries "packages for things you count, weight for things you weigh") because `PurchaseEvent.Quantity` is packages for a counted item and a weight for a weight item. §13.1's rule is display-follows-`DefaultUnit`, so this phase threads it and the column starts saying "4 packages" / "2.34 lb" per row. ONE formatting helper shared with the count's own display, or the two surfaces drift — Not complete
