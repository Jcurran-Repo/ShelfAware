...
- [ ] Unit-test suite: thorough review + rebuild — everything covered, useless tests deleted, a strict quality bar throughout, and no test ever weakened to make it pass (a failing test means fix the code or fix the expectation honestly). Page flows included: the bar is TESTED, not walkthrough-verified, so a page-test harness (bUnit or equivalent) is in scope — Planned, 7/30/2026
