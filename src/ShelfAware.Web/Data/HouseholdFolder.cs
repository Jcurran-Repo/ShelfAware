...
public static class HouseholdFolder
{
    ...
    /// <summary>Removes everything a household has filed under <paramref name="root"/>.</summary>
    public static bool DeleteUnder(string root, string householdId, ILogger logger) =>
        Delete(Path.Combine(root, For(householdId)), logger);

    /// <summary>Removes a directory tree if it's there, reporting rather than throwing when it won't go.
    /// Every caller is deleting files derived from data that is already gone, so failing here must not
    /// fail the operation — it would only invite the user to press the button again.</summary>
    public static bool Delete(string folder, ILogger logger)
    {
        try
        {
