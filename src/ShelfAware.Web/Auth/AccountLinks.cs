...
/// Base64Url-encoded token). Two senders reach it — ForgotPassword (a forgotten password, or recovering a
/// lost activation link) and a new-account ACTIVATION from registration (the same page: setting the password
/// there also confirms the address) — so a hand-built URL in each would be two chances for the path or a
/// parameter name to drift, and a drift there is a dead link nobody can tell is dead. Centralising the shape
/// means every sender and the ResetPassword reader agree by construction.</summary>
