# Build log — the feature arcs, 2026-06 to 2026-09

Archived from `CLAUDE.md` on 2026-09-19, when that file reached 410 KB and was being read in full on
every session. This is **reference**, not instruction: open it when you are working in an area it
covers, not before every task. The rules that bind every session stayed in `CLAUDE.md`.

Each numbered item is one arc — what shipped, what the review gate found, and the constraint the code
can't say for itself. The ⚠️ marks are the expensive ones: a rule that was written down and then broken
anyway, sometimes by the session that had just read it. Where one of those could be held by a test
instead of a paragraph, it should be — see `docs/remediation-plan.md` §1.

The full pre-split file, including the sections that moved elsewhere, is preserved verbatim at
`docs/journal/CLAUDE-2026-09-19.md`.

**Where the rest went:** architecture and data model → `docs/architecture.md` · toolchain, shell and
deploy gotchas → `docs/environment.md` · the voice engine → `docs/features/voice.md` · recipes, tags
and substitutes → `docs/features/recipes.md` · the open backlog → `docs/backlog.md`.

---

## Build state as of the split (historical — the live one is in `CLAUDE.md`)

⚠️ This table was last edited 2026-07-07 and had drifted by the time of the split: it still named
`docs/accuracy.png` as outstanding when that file had existed for weeks. Kept as written, because a
record of what the project believed about itself is part of the record. Do not act on it.
