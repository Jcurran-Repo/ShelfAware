...
# as root. The models live in /opt, root-owned and writable by nobody else; the directory is pinned by
# inode and its parent checked, so that is verified rather than assumed. The archive is downloaded and
# unpacked in a fresh root-only directory inside it, and moved in with one rename only once verified,
# whole and root-owned. Why each of those matters is in deploy-moonshine.md step 1.
{ command -v bzip2 >/dev/null || { apt-get update && apt-get install -y bzip2; }; } \
  && M=/opt/shelfaware-models && V=kokoro-int8-en-v0_19 \
  && mkdir -p "$M" && cd -P "$M" \
...
```bash
dotnet run --project tools/VoiceCheck -- kokoro /opt/shelfaware-models/kokoro-int8-en-v0_19 /tmp/kokoro-check.wav
```
...
/tmp/kcheck/VoiceCheck kokoro /opt/shelfaware-models/kokoro-int8-en-v0_19 /tmp/kokoro-check.wav
...
Speech__Kokoro__ModelDirectory=/opt/shelfaware-models/kokoro-int8-en-v0_19
